			...
			<input type="text" class="input-block-level" id="lastname" name="lastname" placeholder="Other Name" required>
			<label>Level</label>
			...
			jQuery("#signin_student").submit(function(e){
					...
					$.ajax({
						...
						if(html=='true')
						{
						$.jGrowl("Welcome to FPB E-Learning System", { header: 'Sign up Success' });
						var delay = 2000;
							setTimeout(function(){ window.location = 'dashboard_student.php'  }, delay);  
						}else if(html=='false'){
							$.jGrowl("student does not found in the database Please Sure to Check Your Matric Number or Firstname, Other name and the Level You Belong. ", { header: 'Sign Up Failed' });
						}
						}
